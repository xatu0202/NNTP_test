...
const setHandler = () => sock.addEventListener("message", handleMessage, {
  once: true
})
const handleMessage = msg => {
  ...
  switch (responseCode) {
    ...
    case "215":
      ...
      if (!hasMessageEnd(msg.data)) sock.addEventListener("message", nextMessage(msg.data, listReceived), {
        once: true
      })
    ...
    case "220":
      ...
      if (!hasMessageEnd(msg.data)) sock.addEventListener("message", nextMessage(msg.data, articleReceived), {
        once: true
      })
...
const nextMessage = (prevMsg, callback) => msg => {
  ...
  if (!hasMessageEnd(msg)) sock.addEventListener("message", nextMessage(msg, callback), {
    once: true
  })
  else callback(msg)
}
